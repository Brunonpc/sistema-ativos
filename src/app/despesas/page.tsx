...
export default function DespesasPage() {
  ...
  async function carregarTabela() {
    ...
    if (tipoData === "MES" && mesFiltro) {
      ...
      const uDia = new Date(Number(anoStr), Number(mesStr), 0)
        .toISOString()
        .split("T")[0];
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">
          Gerencie seus custos operacionais e administrativos.
        </p>
      </div>
      <Card>
        ...
        <CardContent className="pt-6">
          <form onSubmit={salvarDespesa} className="space-y-5">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <input
                  required
                  type="text"
                  placeholder="Ex: Marina, Limpeza, Luz..."
                  value={descricao}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-slate-50 rounded-lg border border-slate-200">
              <div>
                <label className="text-sm font-bold text-red-600 mb-1 block">
                  Valor (R$)
                </label>
                <input
                  required
            ...
            <div className="flex justify-end">
              <button
                type="submit"
                disabled={carregando}
                className="flex items-center gap-2 bg-[#dc2626] hover:bg-[#b91c1c] text-white px-6 py-2.5 rounded-md text-sm font-medium transition shadow-sm disabled:opacity-50"
              >
      ...
      <Card>
        ...
        <CardContent className="p-0">
          <div
            className={`overflow-x-auto transition-opacity duration-300 ${carregandoTabela ? "opacity-40" : "opacity-100"}`}
          >
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      ...
                      <div className="text-xs text-slate-400 mt-0.5">
                        {d.categorias_despesa?.dre_grupos?.nome}
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex justify-center items-center gap-3">
                        {d.status === "PENDENTE" && (
                          <button
                            onClick={() => marcarComoPago(d.id)}
                            title="Confirmar pagamento"
                            className="flex items-center gap-1 bg-green-50 text-green-700 hover:bg-green-100 border border-green-200 px-2.5 py-1 rounded-md text-xs font-bold transition-colors"
                          >
